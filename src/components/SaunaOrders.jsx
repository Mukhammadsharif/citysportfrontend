...
import CreateSaunaOrder from "./CreateSaunaOrder";
...
export default function SaunaOrders() {
    ...
    const emptyRows =
        page > 0 ? Math.max(0, (1 + page) * rowsPerPage - response?.filter(item => item.type === 'sauna')?.length) : 0;
    ...
    return (
        <div style={{display: 'flex', flexDirection: 'column', width: '100%'}}>
            <CreateSaunaOrder filterParams={filterParams} setFilterParams={setFilterParams}/>
                ...
                <Table sx={{minWidth: '100%'}} aria-label="simple table">
                    ...
                    <TableBody>
                        {(rowsPerPage > 0
                                ...
                                : response
                        )?.filter(item => item.type === 'sauna').toReversed().map((row) => (
                            <TableRow
                                key={row.id}
                                sx={{
                    ...
                    <TableFooter>
                        <TableRow>
                            <TablePagination
                                ...
                                colSpan={10}
                                count={response?.filter(item => item.type === 'sauna')?.length}
